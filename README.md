...
- 已注册并可批量导出的因子：149 个，其中 23 个使用当前 20 只研究股票的日度横截面计算。
- 因子数据采用一因子一文件的长表结构：`trade_date,trade_symbol,factor_value`。
- 行情数据包含前复权 OHLC、同尺度 `vwap`、原始成交量和成交额，以及沪深 300 基准开收盘价。

完整清单见 `factor.py` 中的 `FACTORS` 注册表，以及最新更新记录。
...
# 基于 data/price/ 中的行情生成 149 份因子长表
...
- [最新 149 因子与横截面计算扩展](docs/updates/2026-07-23-149-factor-panel-expansion.md)
- [架构决策记录](docs/adr/)
...
- `vwap` 按日成交额/成交量计算，再转换到前复权价格尺度；它不是独立的逐笔成交重建结果。
- 横截面 `RANK` 目前只在固定的 20 只研究股票内计算，不等同于报告中的全 A 非 ST 股票池。
- 149 个因子可运行，但大多数尚缺独立数值基准测试；其余 42 个因子仍在规划或实现中。
